<script lang="ts">
	...
	let selectedRankStr: string = regionRankOptions[PurpleRegion.AelioIntruders][1].toString();

	$: partySizeOptions =
		$purplesForm.region === PurpleRegion.AelioIntruders ? [1, 2, 4, 8] : [1, 2, 4];
	...
	const regionChanged = () => {
		const rankOptions = regionRankOptions[$purplesForm.region];
		selectedRankStr = rankOptions[rankOptions.length - 1].toString();
		$purplesForm.rank = parseInt(selectedRankStr);
	};
</script>
...
<div class="grid grid-cols-1 gap-2 md:grid-cols-4">
	<div class="md:col-span-3">
		<PartySizeOptions sizes={partySizeOptions} />
	</div>
	<CurrentPatchLabel />
</div>
<div class="grid grid-cols-1 gap-2 md:grid-cols-3">
	<Select
		...
		options={[
			...
			{ label: 'Aelio Intruders', value: PurpleRegion.AelioIntruders },
		]}
