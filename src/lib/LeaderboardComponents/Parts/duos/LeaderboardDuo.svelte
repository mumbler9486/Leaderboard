<script>
    import SoloPurpleFull from "$lib/LeaderboardComponents/Rows/DuoPurpleFull.svelte";
    import PurplesoloModalRuninfo from "$lib/LeaderboardComponents/Modals/PurpleDuo/purpleduo_modal_runinfo.svelte";
  ...
  export async function reloadData() {
    ...
    const response = await fetch('/ngs-api/PurpleDuo' + params, {
        method: 'GET',
        headers: {
          ...
          <table class="table table-compact table-zebra w-full">
            ...
            <tbody>
              {#each dataStorage as data}
                ...
                <SoloPurpleFull on:openRunInfo={runInfoOpen} time={data.shared.Time} runID={data.shared.RunID} notes={data.shared.Notes} myRank={func} player1={data.p1} player2={data.p2}/>
                {/each}
              {/each}
            </tbody>
            {/if}
