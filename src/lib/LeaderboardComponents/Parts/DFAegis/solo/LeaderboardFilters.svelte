<script>
  ...
  function updateURL() {
    ...
    if (
      ...
      (filters.trigger == 1 || filters.trigger == 0)
    ) {
    ...
    if (
      filters.server != null &&
      (filters.server === "japan" || filters.server === "global")
    ) {
...
>
  ...
  <div class="flex flex-row flex-wrap place-content-center items-stretch">
    <div class="m-1 md:flex-1">
      <label
        ...
        on:click={() => modalFilters.openRefresh()}
        ><i class="bi bi-funnel mr-2" />Filters</label
      >
    </div>
  ...
  <div
    class="flex flex-row flex-wrap place-content-center gap-1 md:place-content-start"
  >
    <!-- SOLO PURPLE FILTER DISPLAYS -->
    {#if type == "solo"}
      {#if filters.class && allowedClasses.includes(filters.class)}
        <span
          class="badge badge-lg rounded border border-neutral-content/25 py-4"
        >
          <img
            src="/icons/class/class-{filters.class}.png"
            alt={filters.class}
            class="mr-1"
          />
          {filters.class.charAt(0).toUpperCase() +
            filters.class.slice(1).toLowerCase() +
            "s Only"}
          <button
            class="btn-square btn-xs btn ml-1 rounded"
            on:click={() => clearFilter("class")}
          >
            <svg
      ...
      {#if filters.buff && allowedBuffs.includes(filters.buff)}
        ...
        >
          <img
            class="mr-1 inline"
            src={buffIcon[filters.buff]}
            alt={buffName[filters.buff]}
          />
          Support: {buffName[filters.buff]}
          <button
            class="btn-square btn-xs btn ml-1 rounded"
            on:click={() => clearFilter("buff")}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6"
      ...
      {#if filters.server && (filters.server === "global" || filters.server === "japan")}
        <span
          class="badge badge-lg rounded border border-neutral-content/25 py-4"
        >
          Server: {filters.server.charAt(0).toUpperCase() +
            filters.server.slice(1).toLowerCase()}
          <img
            src="/icons/server/server-{filters.server}.png"
            alt="Global"
            class="ml-1"
          />
          <button
            class="btn-square btn-xs btn ml-1 rounded"
            on:click={() => clearFilter("server")}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6"
