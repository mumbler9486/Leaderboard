<script lang="ts">
    ...
    onMount(async () => {
        ...
        generateNames('Player1',data.PlayerID,data.PlayerName,data.PlayerCName,data.RunCharacter,data.PlayerPrefN,data.PlayerNameColor1,data.PlayerNameColor2,data.PlayerNameType);
        generateNames('Submitter',data.SubmitterID,data.SubmitterName,data.SubmitterCName,data.SubmitterCName,data.SubmitterPrefN,data.SubmitterNameColor1,data.SubmitterNameColor2,data.SubmitterNameType);
        generateWeaponImages(data.WeaponInfo);
        notes = noteGenerate(data.Notes);
        time = timeSplit[0] + "h " + timeSplit[1] + "m " + timeSplit[2] + "s";
        subTime = new Date(data.SubmissionTime).toDateString() + ' @ ' + new Date(data.SubmissionTime).toTimeString().slice(0,8);
    ...
    input.forEach(weapon => {
        if(weapon != null && weapon != '') {
            result = result + '<div class="tooltip tooltip-info font-semibold" data-tip="' + weaponNameMap[weapon] + '"><img src="' + weaponIconMap[weapon] + '" class="inline object-none w-[16px]"></div>' + ' ';
        }
    ...
    function generateNames(playerID,pid,player,character,run,pref,c1,c2,ntype) {
        ...
        if (pid != 106 && pid != 107) {
            ...
            case 1:
                ...
                break;
            case 2:
                cssOutput += `background: -webkit-linear-gradient(0deg, #` + c1 + `, #` + c2 + `);
                            -webkit-background-clip: text;
                            -webkit-text-fill-color: transparent;`;
                break;
            case 3:
                cssOutput += `color: #` + c2 + `; text-shadow: 0px 0px 5px #` + c1 + `, 0px 0px 5px #` + c1 + `, 0px 0px 5px #` + c1 + `;`;
                break;
            default:
                break;
            }
...
<tr class="hover border-t border-t-secondary/20">
    <th><span class="items-center flex"><span style={PInfo['Player1'].CSS} class="truncate transition ease-in-out hover:brightness-125" on:click={openRunMessage}>{PInfo['Player1'].PrimaryName}</span></span>{#if PInfo['Player1'].SecondaryName != null && (Number(data.PlayerID) != (106) && Number(data.PlayerID) != (107))}<p class="mt-1 text-xs">{PInfo['Player1'].SecondaryName}</p>{/if}</th>
    <td class="text-center">{triggerTypeMap[data.Drill]}</td>
    <td class="text-center"><img class="inline mr-1" src={buffIcon[data.Support]} alt={buffName[data.Support]}>{buffName[data.Support]}</td>
    ...
    <td><span class="items-center flex"><span style={PInfo['Submitter'].CSS} class="truncate transition ease-in-out hover:brightness-125" on:click={openRunMessage}>{PInfo['Submitter'].PrimaryName}</span></span>{#if PInfo['Submitter'].SecondaryName != null && (Number(data.SubmitterID) != (106) && Number(data.SubmitterID) != (107))}<p class="mt-1 text-xs">{PInfo['Submitter'].SecondaryName}</p>{/if}</td>
