...
<div class="modal" id="PurpleSolo_RunInfoModal">
	<div class="modal-box relative max-h-full max-w-6xl rounded">
		<div class="flex flex-row">
			<span class="flex-1 self-center text-3xl font-light md:text-4xl">Run Information</span>
				...
				<div class="flex grow flex-col gap-1">
					...
					>
						<div class="flex flex-col text-center">
							<span class="text-lg font-semibold"><i class="bi bi-youtube" /> POV Link(s):</span>
							{#if player1.LinkPOV}
					...
					<div class="flex flex-col gap-2 p-2 md:flex-row md:gap-0">
						<div class="flex basis-full flex-col justify-center">
							...
							{#if player1.NameOut1 !== player1.NameOut2 && player1.PlayerID !== ('106' || '107')}
								<p class="flex place-self-center text-sm text-base-content/50 md:ml-1">
									({player1.NameOut2})
								</p>
							{/if}
							<div class="divider m-0" />
		...
		<div class="modal-action">
			{#if d.shared !== undefined && !processing}
				<label on:click={denyRun} class="btn-outline btn-error btn rounded md:btn-sm">Deny</label>
				<label on:click={approveRun} class="btn-outline btn-success btn rounded md:btn-sm"
					>Approve</label
				>
				<label on:click={closeModal} class="btn-outline btn-secondary btn rounded md:btn-sm"
					>Close</label
				>
			{/if}
		</div>
