...
<tr class="hover border-t border-t-secondary/20">
    <td><span class="flex items-center"><img class="inline mr-1" src={mainClass} alt={mainClassAlt}><span style={cssOutput} class="truncate">{primaryNameOutput}</span></span>{#if secondaryNameOutput != null && PID != (106 || 107)}<p class="mt-1 text-xs">{secondaryNameOutput}</p>{/if}</td>
