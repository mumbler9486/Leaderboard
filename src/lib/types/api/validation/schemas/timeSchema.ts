...
export const yupRunTimeMapped = (validTimesMap: Record<string, number>) => {
	return runTimeSchema.test(
		...
		(runTime, ctx) => {
			...
			const isValid = runSeconds <= maxSeconds;
			if (!isValid) {
				return ctx.createError({
					message: `Run time cannot be more than ${maxTimeLabel}`,
				});
			}
