<script lang="ts">
  ...
  let storedUser = browser
    ? localStorage.getItem("loginreference-last") ?? null
    : null;
  let storedName = browser
    ? localStorage.getItem("playerreference-name") ?? null
    : null;
  let storedStyle = browser
    ? localStorage.getItem("playerreference-style") ?? null
    : null;
  let userInfo = null;
  ...
  onMount(async () => {
    ...
    if (
      ...
      localStorage.setItem(
        "loginreference-last",
        userInfo.userDetails.toString()
      );
      localStorage.setItem(
        "playerreference-name",
        MyPlayer.PlayerName.toString()
      );
      localStorage.setItem(
        "playerreference-style",
        MyPlayer.PlayerStyle.toString()
      );
      storedUser = browser
        ? localStorage.getItem("loginreference-last") ?? null
        : null;
  ...
  function storeLocale(localeReference) {
    let consentPreferences =
      (localStorage.getItem("consent-preferences") ?? false) === "true";
    ...
    <div class="navbar">
      ...
      <div class="dropdown-end dropdown ml-auto md:hidden">
        ...
        >
          ...
          <li>
            <a
              href="/leaderboard/run/purple/solo"
              tabindex="0"
          ...
          <li>
            <a
              href="/leaderboard/run/purple/duo"
              tabindex="0"
              style="border-radius: 0px"
            >
              Duo
          ...
          <li>
            <a
              href="/leaderboard/run/purple/party"
              tabindex="0"
              style="border-radius: 0px"
            >
              Party
          ...
          <li>
            <a
              href="/leaderboard/run/dfa/party"
              tabindex="0"
              style="border-radius: 0px"
            >
              Multi-Party
          ...
          {#if userInfo != null && (userInfo.userRoles.includes("moderator") || userInfo.userRoles.includes("administrator"))}
            <li>
              <a
                href="leaderboard/moderator/submissions"
                tabindex="0"
                style="border-radius: 0px"
              >
                <i class="bi bi-inboxes mr-0" />{$t(
                  "navigation.modadmin.submissionQueue"
                )}
              </a>
            </li>{/if}
          {#if userInfo != null && userInfo.userRoles.includes("user")}
          ...
          {#if userInfo != null && userInfo.userRoles.includes("user")}
            <li>
              <a href="/logout" tabindex="0" style="border-radius: 0px">
                <i class="bi bi-box-arrow-right mr-0" />{$t(
                  "navigation.logout"
                )}
              </a>
            </li>
      ...
      <div class="navbar-end hidden md:flex">
        <div class="dropdown-end dropdown">
          ...
          >
            <span class="m-2 flex text-center text-xs font-semibold uppercase"
              ><img src="/icons/quests/trigger.png" class="mr-2" />Purple
              Triggers</span
            >
            <li>
              <a
        ...
        {#if userInfo != null && userInfo.userRoles.includes("user")}
          <a
            href="/leaderboard/submit"
            class="btn-ghost no-animation btn rounded-none"
          >
            <i class="bi bi-envelope-paper mr-2" />
            <span>{$t("navigation.submitRun")}</span>
          </a>
          ...
          <div class="dropdown-end dropdown">
            ...
            {:else}
              <label
                tabindex="0"
                class="btn-ghost no-animation btn rounded-none"
                ><span class="flex font-bold" style={storedStyle}
                  >{storedName}</span
                ><i class="bi bi-caret-down ml-2" /></label
              >
            {/if}
            <ul
              tabindex="-1"
            ...
            >
              <li>
                <a href="/profile" tabindex="0" style="border-radius: 0px">
                  <i class="bi bi-person-vcard mr-0" />{$t(
                    "navigation.profile"
                  )}
                </a>
              </li>
      ...
      <a href="/setup" class="alert alert-warning m-2 shadow-lg">
        <div>
          ...
          <span class="font-bold"
            >You must finish user setup to upload runs and access certain other
            features! Click or Tap here to do so!</span
          >
        </div>
      </a>
    {/if}
